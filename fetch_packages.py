    ...
    while retry_count <= _RETRIES:
        for url in urls:
            ...
            if "{{ site_mirror }}" in url:
                if not ARGS['site-mirror']:
                    continue
                url = url.replace("{{ site_mirror }}", ARGS['site-mirror'])
            subprocess.call([wgettool, '--no-check-certificate', '-O', pkg, url.text, '--timeout=10'])
            md5sum = FindMd5sum(pkg)
            if ARGS['verbose']:
...
def parse_args():
    ...
    parser.add_argument("--site-mirror", required=False, default=None)
